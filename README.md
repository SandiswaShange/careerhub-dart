...
## Question 4
The first named constructor represents a job that is specifically created for the CareerHub training programme, where the job automatically has a predefined status, training category, and eligibility requirements that together define a valid business state rather than just assigning default values. The second named constructor represents a job that has already been filled or closed, ensuring that the object is created with the correct combination of status, closing details, and any related fields that reflect a completed recruitment process. These business scenarios enforce meaningful domain states that the default constructor should not be responsible for creating, making the named constructors valuable because they encapsulate valid CareerHub-specific situations instead of simply pre-filling convenience values.